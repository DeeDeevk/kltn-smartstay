...
describe('buildSystemPrompt', () => {
  it('lấy "hôm nay" theo giờ Việt Nam, không theo UTC', () => {
    ...
    const prompt = buildSystemPrompt({ now: new Date('2026-09-18T20:00:00Z') });
  ...
  it('giữ đúng ngày khi đã là buổi chiều giờ Việt Nam', () => {
    ...
    const prompt = buildSystemPrompt({ now: new Date('2026-09-18T08:00:00Z') });

    expect(prompt).toContain('Thứ Sáu, ngày 2026-09-18');
  });

  it('nhắc model mời đăng nhập khi khách chưa đăng nhập, và không nhắc khi đã đăng nhập', () => {
    const guestPrompt = buildSystemPrompt({ isGuest: true });
    const memberPrompt = buildSystemPrompt({ isGuest: false });

    expect(guestPrompt).toContain('CHƯA ĐĂNG NHẬP');
    expect(memberPrompt).not.toContain('CHƯA ĐĂNG NHẬP');
  });
});
